...
namespace AVEncoder
{
    namespace
    {
        ...
        NV_ENC_PARAMS_RC_MODE ToNVRateControl(FVideoEncoder::RateControlMode Mode)
        {
            switch (Mode)
            {
            case FVideoEncoder::RateControlMode::CONSTQP:
                return NV_ENC_PARAMS_RC_CONSTQP;
            case FVideoEncoder::RateControlMode::VBR:
                return NV_ENC_PARAMS_RC_VBR;
            case FVideoEncoder::RateControlMode::CBR:
            default:
                return NV_ENC_PARAMS_RC_CBR;
            }
        }

        NV_ENC_MULTI_PASS ToNVMultiPass(FVideoEncoder::MultipassMode Mode)
        {
            switch (Mode)
            {
            case FVideoEncoder::MultipassMode::QUARTER:
                return NV_ENC_TWO_PASS_QUARTER_RESOLUTION;
            case FVideoEncoder::MultipassMode::FULL:
                return NV_ENC_TWO_PASS_FULL_RESOLUTION;
            case FVideoEncoder::MultipassMode::DISABLED:
            default:
