...
FText FCameraShakeSourceShakeTrackEditor::GetDisplayName() const
{
	return LOCTEXT("CameraShakeSourceShakeTrackEditor_DisplayName", "Camera Shake Source");
}
...
bool FCameraShakeSourceShakeTrackEditor::OnShouldFilterCameraShake(const FAssetData& AssetData)
{
	...
	{
		TSubclassOf<UCameraShakeBase> CameraShakeClass = *(SelectedObject->GeneratedClass);
		if (const UCameraShakeBase* CameraShakeCDO = Cast<UCameraShakeBase>(CameraShakeClass->GetDefaultObject(false)))
		{
